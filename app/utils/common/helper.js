...
module.exports = {
  ...
  checkImageExtension: function(fieldName) {
    return (
      'The ' +
      fieldName +
      ' field must have extension jpg, jpeg, png, gif or svg.'
    );
  },
  ...
  checkIfArray: function(fieldName) {
    return (
      'The ' + fieldName + ' must not be empty and it should be a valid array.'
    );
  },
  ...
  checkLength: function(fieldName, min, max) {
    ...
    if (min == max)
      ...
      return (
        'The ' + fieldName + ' may not be greater than ' + max + ' characters.'
      );
    else
      return (
        'The ' +
  ...
  randomStr: function(length) {
    ...
    for (var i = 0; i < length; i++)
      randomText += possible.charAt(
        Math.floor(Math.random() * new Date().getSeconds().toString())
      );
  ...
  generateUserName: function(name) {
    ...
    var code = '';
    if (userNameLength == 3) code = userName + '' + helpers.randomStr(7);
    else if (userNameLength == 4) code = userName + '' + helpers.randomStr(6);
    else code = userName.substr(0, 5) + '' + helpers.randomStr(5);
  ...
  findOneData: function(
    ...
    collection.findOne(values).exec(function(err, result) {
      if (err) {
        ...
        if (collectionName === 'User') callback(result);
        else {
  ...
  findOneAndUpdateData: function(
    ...
    collection
      .findOneAndUpdate(findQuery, setValues, {
        new: outputType,
        runValidators: true
      })
  ...
  findByIdData: function(res, collectionName, collection, value, callback) {
    collection.findById({ _id: value }).exec(function(err, result) {
      if (err) {
  ...
  countRecords: function(res, collection, filterQuery, callback) {
    collection.count(filterQuery).exec(function(err, result) {
      if (err) {
        log('Error in helpers.findData function : ', err.message);
        helpers.createResponse(
          res,
   ...
   * @return callback
   */
  findOneUpdateOrInsert: function(
    res,
    model,
    modelName,
    query,
    ...
    model.findOneAndUpdate(query, update, options, function(err, result) {
      if (err) {
        log(
          'Error in helpers.findOneUpdateOrInsert function : ' +
            modelName +
            '=>',
          err.message
        );
